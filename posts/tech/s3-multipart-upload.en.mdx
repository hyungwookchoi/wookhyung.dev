...
## 7. Uploading to S3 from Frontend: Presigned URLs

Now that you understand how multipart upload works, you might wonder how to implement it in a real web application. To upload files directly to S3 from the frontend, you need to understand **Presigned URLs**.

### Why Presigned URLs Are Needed

Accessing S3 requires AWS Access Key and Secret Key. But exposing these credentials to the browser creates serious security issues.
...
Presigned URLs solve this by generating **URLs with temporary access permissions**, allowing file uploads to S3 without credentials.

### How It Works

The backend generates a Presigned URL using the AWS SDK.
...
The URL contains a **Signature**, allowing anyone with this URL to perform the specified operation only for the designated time. The frontend uploads directly to S3 using this URL.
...
For large files, you can generate a Presigned URL for each part and upload them in parallel.

```javascript
// Backend: Initialize multipart + generate Presigned URLs for each part
...
The frontend splits the file and uploads in parallel.
...
You could upload files through the backend. But consider uploading a 100MB file.

**Via Backend:**
- User → Backend: 100MB transfer
- Backend → S3: 100MB transfer
- Total traffic: 200MB, backend server load

**With Presigned URL:**
- User → Backend: URL request (few KB)
- User → S3: 100MB direct transfer
- Total traffic: 100MB, no backend load

Since files bypass the backend, both server costs and response times improve.

### Security

Presigned URLs have these security characteristics:

- **Time limit**: Set validity period with `expiresIn` (max 7 days)
- **Operation restriction**: Can limit to PUT only, or GET only
- **Path restriction**: Only accessible to specified file paths
- **No credential exposure**: URL contains only signature, not actual Access Key

Even if a URL is leaked, it becomes useless after the specified time expires, and only designated operations can be performed, making it much safer than exposing credentials directly.
