...
summary: 'How React blocks scripts with innerHTML and parser-inserted flag, and fixing TanStack Router execution issues'
date: '2026-01-10'
...
## 💡 First Solution: Dynamic Script Injection
...
## 🚨 Second Problem: Duplicate Execution in SSR
...
if (attrs?.src && typeof attrs.src === 'string') {
  // URL normalization (relative path → absolute path)
  const normSrc = (() => {
    try {
...
- "Why doesn't it work?" → Identifying root causes
- "How can we solve this?" → Setting the right direction
